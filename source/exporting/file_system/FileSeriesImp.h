...
{
    return p_files.size() + p_exsistedIndexes.size();
}
...
typename FileSeries<FileType>::Index FileSeries<FileType>::highestIndex() const
{
    ...
    const auto existedIndex = (p_exsistedIndexes.empty()) ? 0 : *p_exsistedIndexes.rbegin();
...
{
    return p_exsistedIndexes.find(index) != p_exsistedIndexes.end();
}
...
unsigned long FileSeries<FileType>::removeAll(const bool ownedOnly)
{
    ...
    if(!ownedOnly) {
        recoverExistedFiles();
        for(auto itr = p_exsistedIndexes.begin(); itr != p_exsistedIndexes.end(); itr++, ans++ ) {
            std::remove(fullName(*itr, true).c_str());
        }
        p_exsistedIndexes.clear();
    }
    p_warningSent = false;
    return ans;
}
...
{
    p_exsistedIndexes = readExistedFiles();
    return p_exsistedIndexes.size();
}
